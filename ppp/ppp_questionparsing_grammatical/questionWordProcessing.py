    ...
    try:
        if 'identity' in predList:
             if w in strongQuestionWord or isinstance(nf.subject, Resource) or isinstance(nf.object, Resource): # strong qw or triple of depth 1
                 wisWords=[x for x in wisMap[w]]
                 wisWords.insert(0,predList[0]) #append the original pred
                 return Triple(nf.subject, List([Resource(x) for x in wisWords]), nf.object) # !! Other info lost (type...) (inverse_predicate: not relevant)
