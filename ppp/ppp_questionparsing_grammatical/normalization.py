...
def verbStandardize(tree):
    ...
    return (' '.join(wSplit), pastPart),original
...
def buildPredicateVerb(tree):
    ...
    """
    lem,original= verbStandardize(tree) # (v1, v2) where v1=lemmatize(v) and v2 is the past participle of v (v = verb of wordList)
    
    lDirect = [original, lem[1]] # the past participle is always a predicate
    lInverse = []
